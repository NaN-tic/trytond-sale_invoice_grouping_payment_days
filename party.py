...
class Party(metaclass=PoolMeta):
    ...
    def validate(cls, parties):
        ...
        for p in parties:
            p.check_customer_payment_days()

    def check_customer_payment_days(self):
        if self.sale_invoice_grouping_period != 'customer_payment_days':
            return
        if not self.customer_payment_days:
            raise UserError(
                gettext(
